    ...
    if (configs.length === 0) {
      await runQuery(
        `INSERT INTO "SystemConfig" (id,"minBet","maxBet","roundDurationBetting","roundDurationCalcul","roundDurationReveal","isFreeEnabled","isCashEnabled","historyLength","isMaintenanceMode","inviteRewardInviter","inviteRewardInvitee","dailyInviteLimit","isReferralActive")
         VALUES ('singleton',10,10000,25,4,0,true,true,20,false,500,200,5,true)
         ON CONFLICT (id) DO UPDATE SET "roundDurationBetting"=25,"roundDurationCalcul"=4,"roundDurationReveal"=0`
      );
      console.log("✅ SystemConfig seeded.");
    } else {
      await runQuery(`UPDATE "SystemConfig" SET "roundDurationBetting"=25,"roundDurationCalcul"=4,"roundDurationReveal"=0 WHERE id='singleton'`);
